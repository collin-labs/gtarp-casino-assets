...
export default function VideoGrid({ grid, tumbles, spinning, multiplier, onTumbleComplete, turbo = false }: VideoGridProps) {
  ...
  // Animação de shuffle durante spin (símbolos trocam rápido coluna por coluna)
  useEffect(() => {
    if (spinning && !isTumbling) {
      setIsShuffling(true);
      shuffleRef.current = setInterval(() => {
        setDisplayGrid(prev => prev.map((col, ci) =>
          col.map(() => ALL_SYMS[Math.floor(Math.random() * ALL_SYMS.length)])
        ));
      }, 150);
    } else if (!spinning && shuffleRef.current) {
  ...
  useEffect(() => {
    ...
    const processTumble = async (idx: number) => {
      ...
      // 2. Highlight clusters — pausa dramática para jogador ver
      ...
      await delay(turbo ? 600 : 1200);

      // 3. Explodir células
      setExplodingCells(hlSet);
      setHighlightedCells(new Set());
      await delay(turbo ? 200 : 400);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        filter: isShuffling ? "blur(0.8px)" : "none",
