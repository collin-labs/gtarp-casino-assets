...
import type { RouletteView, RouletteMode, Bet, SpinResult, HistoryEntry } from "./RouletteGame";
...
export default function RouletteTable({
  ...
  useEffect(() => {
    const calc = () => setWheelSize(Math.min(window.innerWidth * 0.52, 640));
    calc();
          ...
          {history.slice(0, 10).map((entry, i) => (
            <motion.div
              ...
              style={{
                ...
                borderRadius: "50%",
                background: getHistoryColor(entry.number),
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                border: `1px solid ${entry.isLucky ? "#FFD700" : entry.number === 0 ? "#00E676" : "rgba(255,255,255,0.2)"}`,
                fontSize: "clamp(8px, 0.9vw, 12px)",
                fontWeight: 700,
                color: entry.number === 0 ? "#000" : "#FFF",
                fontFamily: "'JetBrains Mono', monospace",
                boxShadow: entry.isLucky ? "0 0 6px rgba(255,215,0,0.5)" : "none",
              }}
            >
              {entry.number}
            </motion.div>
